...
async function addTaskToBlockchain(content) {
    try {
        ...
        console.log('Task 
        added to blockchain successfully.');
        return true;
    } catch (error) {
...
async function createtodo() {
    ...
    if (input && !localtodolist.includes(input)) {
        ...
        // Blockchain transaction happens asynchronously
        const success = await addTaskToBlockchain(input);
        if (!success) {
            console.error('Failed transaction, removing task from UI');
            count -= 1;
            localtodolist = localtodolist.filter(task => task !== input);
            localStorage.setItem("mainlocaltodolist", JSON.stringify(localtodolist));
            reload(); 
        }
    }
